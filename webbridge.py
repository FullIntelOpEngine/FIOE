    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if session_id:
                ...
                cur.execute(
                    "SELECT useraccess FROM login WHERE username=%s LIMIT 1",
                    (username,),
                )
                row = cur.fetchone()
                cur.close(); conn.close()
...
def _require_session(f):
    ...
    back to a username-existence check.
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            if session_id:
                ...
                cur.close(); conn.close()
                if not row:
                    # Strict behaviour: reject when a session_id is present but invalid
                    return jsonify({"error": "Session expired or invalid"}), 401
            else:
                # Legacy fallback: just confirm the username exists
